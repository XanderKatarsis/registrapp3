...
export class AuthServiceService {
  ...
  async signInWithPhoneNumber(phoneNumber: string) {
    ...
    const confirmacionResult = await this.ngFireAuth.signInWithPhoneNumber(phoneNumber, appVerifier)
    const verificationCode = window.prompt(phoneNumber + 'Enter the verification code');

    if (verificationCode) {
      const userCredential = await confirmacionResult.confirm(verificationCode);
